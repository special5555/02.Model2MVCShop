...
public class Page {
	...
	public Page( int currentPage, int totalCount,	int pageUnit, int pageSize ) {
		...
		this.maxPage = (pageSize == 0) ? totalCount :  (totalCount-1)/pageSize +1; // 전체 페이지=(한 페이지당 보여지는 게시물수 ==0) ? 총 게시물 수 : (총게시물수-1)/ 한페이지당 보여지는 게시물수   +1
		this.currentPage = ( currentPage > maxPage) ? maxPage : currentPage;		 //현재페이지 = (현재페이지> 전체페이지) 전체페이시 : 현재페이지;
		
		this.beginUnitPage = ( (currentPage-1) / pageUnit ) * pageUnit +1 ; //화면에 보여지는 페이지 번호의 최소수 (( 현재페이지-1) / 하단페이지 번호 화면에 보여지는 수 )*하단페이지 번호 화면에 보여지는 수+1;
	
		
		if( maxPage <= pageUnit ){   // (전체페이지<=하단페이지 번호 화면에 보여지는 수) 
			this.endUnitPage = maxPage;  //화면에 보여지는 페이지 번호의 최대수 
		}else{
